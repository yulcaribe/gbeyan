/* HGBS countries data | version: v1.3.1beta */
(function exposeHGBSCountries(global) {
  ...
  const bySearchText = Object.create(null);
  ...
  countries.forEach(country => {
    const normalized = Object.freeze({
      value: String(country.value || '').trim().toUpperCase(),
      label: String(country.label || '').trim(),
      optionalLabel: country.optionalLabel ?? null
    });

    if (!normalized.value) return;

    byCode[normalized.value] = normalized;
    bySearchText[foldCountryText(normalized.value)] = normalized.value;
    bySearchText[foldCountryText(normalized.label)] = normalized.value;
    if (normalized.optionalLabel) {
      bySearchText[foldCountryText(normalized.optionalLabel)] = normalized.value;
    }
  });

  const normalizedCountries = Object.freeze(Object.values(byCode));

  global.HGBS_COUNTRIES = normalizedCountries;
  global.HGBS_COUNTRY_BY_CODE = Object.freeze(byCode);
  global.HGBS_COUNTRY_DATA = Object.freeze({
    countries: normalizedCountries,
    byCode: global.HGBS_COUNTRY_BY_CODE,
    get(value) {
      return byCode[String(value || '').trim().toUpperCase()] || null;
    },
    findCode(value) {
      return bySearchText[foldCountryText(value)] || '';
    },
    format(value) {
      const country = this.get(value);
      return country ? `${country.value} - ${country.label}` : String(value || '').trim().toUpperCase();
    }
  });
})(window);
