    ...
    private function syncPayload(string $folderId, string $syncKey, bool $getChanges): string
    {
        ...
        if ($getChanges) {
            ...
            $bodyPreference = $this->tag(17, 5,
                $this->tag(17, 6, $this->inlineText((string) ($this->config['body_type'] ?? '1'))) .
                $this->tag(17, 7, $this->inlineText((string) ($this->config['body_truncation'] ?? '32768')))
            );
            $collection .=
                $this->tag(0, 30) .
