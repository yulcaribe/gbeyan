...
function gb_private_page(): string
{
    return <<<'HTML'
<!doctype html>
<html lang="tr">
...
<body><main class="shell">
  ...
  (() => {
    ...
    function render(data,settings){lastData=data;const stats=document.getElementById('stats');stats.replaceChildren();[['Sürüm',data.version],['Uçuş',data.flights.length],['GenDec',data.genDec.length],['Son tarama',date(data.cachedAt)]].forEach(([label,value])=>{const box=document.createElement('div');box.className='stat';const span=document.createElement('span');span.textContent=label;const strong=document.createElement('strong');strong.textContent=text(value);box.append(span,strong);stats.appendChild(box);});
      ...
      table('genDec',['Tarih','Konu','Gönderen','Dosya'],data.genDec,(row,item)=>{cell(row,date(item.date));cell(row,item.subject);cell(row,item.from);cell(row,(item.attachments||[]).map(file=>file.name).join(', '));});
      document.getElementById('jsonData').textContent=JSON.stringify({cachedAt:data.cachedAt,settings:data.settings,folderStatus:data.folderStatus,cleanup:data.cleanup,flights:data.flights,parsed:data.parsed,genDec:data.genDec},null,2);
    }
    ...
    document.getElementById('gateForm').addEventListener('submit',async event=>{event.preventDefault();error.textContent='';open.disabled=true;open.textContent='Kontrol ediliyor…';key=keyInput.value.trim();try{await request('/api/auth/verify');open.textContent='Mailler okunuyor…';await load();keyInput.value='';gate.hidden=true;dashboard.hidden=false;}catch(failure){key='';error.textContent=failure.message;}finally{open.disabled=false;open.textContent='Aç';}});
    ...
    document.getElementById('lock').addEventListener('click',()=>{key='';lastData=null;dashboard.hidden=true;gate.hidden=false;keyInput.focus();});
  })();
</script></body></html>
HTML;
}

function gb_send_private_page(): never
{
    header('Content-Type: text/html; charset=utf-8');
    header('Cache-Control: no-store');
    header('Cross-Origin-Opener-Policy: same-origin');
    header("Content-Security-Policy: default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'");
    echo gb_private_page();
    exit;
}
