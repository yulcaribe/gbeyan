...
const FHY_PARSER_CDN_URL = 'https://cdn.jsdelivr.net/gh/yulcaribe/gbeyan@main/gendec/fhy.js?v=v1.5.0beta';
...
function ensureFhyParserLoaded() {
  if (window.FHYParser && typeof window.FHYParser.parsePdfFile === 'function') {
    console.info('[FHYParser] already loaded', window.FHYParser.version || 'unknown');
    return Promise.resolve(window.FHYParser);
  }

  if (_fhyParserLoadPromise) return _fhyParserLoadPromise;
  ...
  _fhyParserLoadPromise = new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      if (window.FHYParser && typeof window.FHYParser.parsePdfFile === 'function') {
        console.info('[FHYParser] dynamically loaded', window.FHYParser.version || 'unknown');
        resolve(window.FHYParser);
      } else {
        reject(new Error('FHYParser script yüklendi ama window.FHYParser bulunamadı.'));
      }
    };

    script.onerror = () => reject(new Error('FHYParser CDN yüklenemedi: ' + script.src));

    console.info('[FHYParser] loading from CDN', script.src);
  ...
  try {
    ...
    if (!crews.length) {
      const fhyErrorNote = isFreebirdPdf
        ...
        : '';
      setCrewStatus('error', `PDF içinden ekip listesi bulunamadı.${fhyErrorNote} Gendec formatı farklı olabilir.`);
    ...
    const fhySourceNote = fhyResult.matched && fhyResult.pageNo
      ? ` FHY sayfa ${fhyResult.pageNo} eşleşti.`
      : '';
    setCrewStatus('success', `${crews.length} ekip bulundu.${fhySourceNote} Kontrol et, gerekirse düzelt, sonra gönder.`);
...
function parseGendecCrewText(text) {
  ...
  const specialParsers = [
    typeof parseNorwegianCompactCrewLines === 'function' ? parseNorwegianCompactCrewLines : null,
    typeof parseRyanCrewLines === 'function' ? parseRyanCrewLines : null,
    typeof parseSxsCrewLines === 'function' ? parseSxsCrewLines : null
  ].filter(Boolean);

  for (const parserFn of specialParsers) {
    for (const crew of parserFn(rawLines, { lines })) {
      crews.push(crew);
    }
  }
